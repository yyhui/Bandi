function Tabs(arr) {
    this.arr = arr || ['tabs', 'tab-head', 'tab-content', 'selected', 'show'],
        elem = document.getElementById(this.arr[0]);
    
    this.tabs = elem.getElementsByClassName(this.arr[1]);
    this.contents = elem.getElementsByClassName(this.arr[2]);
}

Tabs.prototype.tabEvent = function() {
    var self = this;
    for (var i = 0; i < this.tabs.length; i++) {
        ...
        tab.onclick = function() {
            if (this.className.indexOf(sName) !== -1) return;
...
Tabs.prototype.clearSelect = function() {
    for (var i = 0; i < this.tabs.length; i++) {
...
window.onload = function() {
    var tabs = new Tabs(),
        areaList = new Carousel(
            ['area', 'list', 'item', 'btn', 'span'],
            direction='top'
        ),
        featureList = new Carousel(
            ['features-area', 'features-list', 'feature', 'btn', 'span']
        ),
        group = new Carousel(
            ['group', 'member-list', 'item', 'btn', 'span']
        );
    tabs.tabEvent();
    areaList.init();
    featureList.init();
    ...
    for (var i = 0; i < group.items.length; i++) {
        group.items[i].onmouseover = group.stop.bind(group);
        group.items[i].onmouseout = group.play.bind(group);
    }
};
